...
func TestSaturationDatabaseRepoSQL(t *testing.T) {
	...
	repo := dbrepo.NewRepository(rec)
	record("latency.GetLatencyBySystem", func() {
		_, _ = repo.GetLatencyBySystem(ctx, tenantID, startMs, endMs, dbFilters())
	})

	record("volume.GetOpsBySystem", func() {
		_, _ = repo.GetOpsBySystem(ctx, tenantID, startMs, endMs, dbFilters())
	})

	record("explorer.GetSystemSummariesRaw", func() {
		_, _ = repo.GetSystemSummariesRaw(ctx, tenantID, startMs, endMs)
	})
	record("explorer.GetActiveConnectionsBySystem", func() {
		_, _ = repo.GetActiveConnectionsBySystem(ctx, tenantID, startMs, endMs)
	})

	record("slowqueries.GetSlowQueryPatterns", func() {
		_, _ = repo.GetSlowQueryPatterns(ctx, tenantID, startMs, endMs, dbFilters(), defaultRowCap)
	})

	record("querydetail.GetSummary", func() {
		_, _ = repo.GetSummary(ctx, tenantID, startMs, endMs, queryHash, dbFilters())
	})
	record("querydetail.GetServices", func() {
		_, _ = repo.GetServices(ctx, tenantID, startMs, endMs, queryHash, dbFilters())
	})
	record("querydetail.GetTimeseries", func() {
		_, _ = repo.GetTimeseries(ctx, tenantID, startMs, endMs, queryHash, dbFilters())
	})
	record("querydetail.GetExecutions", func() {
		_, _ = repo.GetExecutions(ctx, tenantID, startMs, endMs, queryHash, dbFilters(), defaultRowCap)
	})

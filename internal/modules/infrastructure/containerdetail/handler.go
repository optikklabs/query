...
func (h *Handler) GetOverview(w http.ResponseWriter, r *http.Request) {
	...
	modulecommon.HandleRangeQuery(w, r, "Failed to query pod overview", func(ctx context.Context, tenantID, startMs, endMs int64) (any, error) {
		return h.Service.GetOverview(ctx, tenantID, pod, startMs, endMs)
	})
}
...
func (h *Handler) GetSeries(w http.ResponseWriter, r *http.Request) {
	...
	rows, known, err := h.Service.GetSeries(r.Context(), tenantID, pod, metricID, startMs, endMs)
	if !known {
		modulecommon.RespondErrorWithCause(w, r, http.StatusBadRequest, errorcode.BadRequest, "unknown metric group", nil)
		return
	}
	if err != nil {
		modulecommon.RespondErrorWithCause(w, r, http.StatusInternalServerError, errorcode.Internal, "Failed to query pod series", err)
		return
	}

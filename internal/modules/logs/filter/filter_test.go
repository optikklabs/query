...
func TestBuildAttrClauseGolden(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			sql:  ` AND attributes_string[@akey_0] = @aval_0`,
		...
		},
		{
			...
			sql: ` AND (attributes_string[@akey_0] = @aval_0 OR (mapContains(attributes_number, @akey_0)` +
		...
		},
		{
			...
			sql: ` AND (attributes_string[@akey_0] = @aval_0 OR (mapContains(attributes_bool, @akey_0)` +
		...
		},
		{
			...
			sql:  ` AND positionCaseInsensitive(attributes_string[@akey_0], @aval_0) > 0`,
			args: []any{key, clickhouse.Named("aval_0", "ab")},
		},
		{
			name: "regex",
			af:   AttrFilter{Key: "user.id", Op: "regex", Value: "^ab.*"},
			sql:  ` AND match(attributes_string[@akey_0], @aval_0)`,
			args: []any{key, clickhouse.Named("aval_0", "^ab.*")},
		},
		{
			name: "gt",
		...
		},
		{
			...
			af:   AttrFilter{Key: "user.id", Op: "eq", Value: "abc"},
			i:    3,
			sql:  ` AND attributes_string[@akey_3] = @aval_3`,

...
func parseHash(w http.ResponseWriter, r *http.Request) (string, bool) {
	...
	if len(values) != 1 || !queryHashPattern.MatchString(strings.TrimSpace(values[0])) {
		modulecommon.RespondErrorWithCause(w, r, http.StatusBadRequest, errorcode.Validation, "hash must be provided once as 16 lowercase hexadecimal characters", nil)
		return "", false
	}
...
func rejectUnknownQueryParams(w http.ResponseWriter, r *http.Request, allowed map[string]struct{}) bool {
	for key := range r.URL.Query() {
		if _, ok := allowed[key]; !ok {
			modulecommon.RespondErrorWithCause(w, r, http.StatusBadRequest, errorcode.Validation, "unknown query parameter: "+key, nil)
			return true
		}
	}
	return false
...
func requiredSingleParam(w http.ResponseWriter, r *http.Request, key string) (string, bool) {
	...
	if len(values) != 1 || strings.TrimSpace(values[0]) == "" {
		modulecommon.RespondErrorWithCause(w, r, http.StatusBadRequest, errorcode.Validation, key+" is required exactly once", nil)
		return "", false
	}
	return values[0], true
}
...
func (h *Handler) GetQueryPerformanceCatalogue(w http.ResponseWriter, r *http.Request) {
	...
	modulecommon.HandleRangeQuery(w, r, "Failed to query database query catalogue", func(ctx context.Context, tenantID, startMs, endMs int64) (any, error) {
...
func parseSeriesScope(w http.ResponseWriter, r *http.Request) (collection, queryHash string, limit int, ok bool) {
	...
	if (len(collectionValues) == 1) == (len(queryHashValues) == 1) {
		modulecommon.RespondErrorWithCause(w, r, http.StatusBadRequest, errorcode.Validation, "exactly one of collection or queryHash is required", nil)
		return "", "", 0, false
	}
	if len(collectionValues) > 0 {
		if len(collectionValues) != 1 || strings.TrimSpace(collectionValues[0]) == "" {
			modulecommon.RespondErrorWithCause(w, r, http.StatusBadRequest, errorcode.Validation, "collection must be a non-empty single value", nil)
			return "", "", 0, false
		}
	...
	if len(queryHashValues) > 0 {
		if len(queryHashValues) != 1 || !queryHashPattern.MatchString(queryHashValues[0]) {
			modulecommon.RespondErrorWithCause(w, r, http.StatusBadRequest, errorcode.Validation, "queryHash must be 16 lowercase hexadecimal characters", nil)
			return "", "", 0, false
		}
	...
	if len(limitValues) > 1 {
		modulecommon.RespondErrorWithCause(w, r, http.StatusBadRequest, errorcode.Validation, "limit must be provided at most once", nil)
		return "", "", 0, false
	}
	if raw := r.URL.Query().Get("limit"); raw != "" {
		parsed, err := strconv.Atoi(raw)
		if err != nil || parsed < 1 || parsed > repository.MaxSeriesLimit {
			modulecommon.RespondErrorWithCause(w, r, http.StatusBadRequest, errorcode.Validation, "limit must be an integer between 1 and 100", nil)
			return "", "", 0, false
		}
		limit = parsed
	}
...
func (h *Handler) GetQueryPerformanceSeries(w http.ResponseWriter, r *http.Request) {
	...
	modulecommon.HandleRangeQuery(w, r, "Failed to query database query series", func(ctx context.Context, tenantID, startMs, endMs int64) (any, error) {
